...
2.  Jazzy can get to play with the balls when they are present as a single unit only and not in any group of size greater than 1. 
...
3.  The length of all the packets/groups should always be an integer.
...
def divide_balls(balls):
    ...
    # for i in range(int(balls/2),1,-1):
    #     if (balls % i == 0):
    #         groups = balls / i
    #         count += 1
    #         if groups == balls:
    ...
    # for i,d in enumerate(len(divisors)+1, start=1):
